    ...
    <section id="projects" className="px-4 py-8 sm:px-6 lg:px-10">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-300">
          A clean project list up front, with the deeper write-up available
          after you click into each project.
        </p>
      </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        {projects.map((project) => (
          <article
            key={project.slug}
            className="group overflow-hidden rounded-[2rem] border border-white/70 bg-white/92 p-5 shadow-sm transition-transform duration-300 hover:-translate-y-1 dark:border-white/10 dark:bg-white/5"
          >
            <div className="rounded-[1.5rem] border border-slate-200/70 bg-[linear-gradient(135deg,rgba(248,250,252,0.92),rgba(207,250,254,0.76),rgba(254,249,195,0.72))] p-4 dark:border-white/10 dark:bg-[linear-gradient(135deg,rgba(15,23,42,0.92),rgba(8,47,73,0.82),rgba(113,63,18,0.62))]">
              {project.img ? (
                <Image
                  src={`/img/${project.img}`}
                  alt={project.name}
                  width={1200}
                  height={900}
                  className="h-52 w-full rounded-[1rem] object-cover object-top"
                />
              ) : (
                <div className="flex h-52 flex-col justify-between rounded-[1rem] bg-slate-950 p-5 text-white dark:bg-slate-900">
